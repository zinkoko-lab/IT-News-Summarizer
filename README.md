...
-   `app/formatter.py`: LINE テキストメッセージ生成（URL なし）
...
-   24 時間フィルタ: `published_at` を UTC 基準で判定
-   取得件数: `global` と `japan` をそれぞれ最大 50 件
-   Top 5 選定: Gemini が各カテゴリから重要度順に 5 件を選定
-   Gemini 入力: タイトルと description のみ（URL は渡さない）
-   LINE 配信: タイトル・要約・元記事 URL を送信
-   出力形式:
    -   `Global Top 5: [タイトル] + [要約]`
    -   `Japan Top 5: [タイトル] + [要約]`
-   障害耐性: 取得失敗時はカテゴリ単位で空配列、Gemini 失敗時は description ベース要約へフォールバック
...
-   LINE Push API は 1 リクエスト最大 5 メッセージのため、クライアント側で分割送信しています。
...
-   `AWS_ROLE_TO_ASSUME` (OIDC で Assume する IAM ロール ARN)
