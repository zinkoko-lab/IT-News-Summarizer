...
class GeminiSummarizer:
    ...
    def _build_prompt(self, articles: list[Article], region: Region, top_n: int) -> str:
        ...
        return (
            ...
            f"上位件数: {top_n}\n"
            "ルール:\n"
            "- 出力言語は【全てミャンマー語（ビルマ語）】です。日本語や英語は一切混ぜないでください。\n"
            "- 技術専門用語は正確に維持する\n"
            "- ソース記事（日本語・英語）の内容は、文脈を踏まえて自然なミャンマー語に意訳する\n"
            "- URLは絶対に含めない\n"
            "- 1記事につき要約は箇条書き1点のみ（ミャンマー語で記述）\n"
            "- タイトルも必ずミャンマー語に翻訳すること\n"
            "- 候補外の事実を創作しない\n"
            "出力は必ず以下の JSON フォーマットのみとし、他のテキストは含めないでください。キーの値は必ずミャンマー語で記述すること。\n"
            '{"items":[{"index":1,"title_my":"[ミャンマー語に翻訳したタイトル]","summary":"-[ミャンマー語で1点に要約した内容]"}]}'
            "\n候補一覧:\n" + "\n".join(lines)
        )
